...
-- Function to convert the circular list starting from a given node into a regular list
toList :: (Eq a) => Node a -> IO [a]
toList startNode = go startNode []
  where
    go currentNode acc = do
      nextNode <- moveForward currentNode
      let newAcc = acc ++ [value currentNode]
      case nextNode of
        Just nextNode' -> do
          sameNode <- nodesAreSame nextNode' startNode
          if sameNode
            then return newAcc  -- Circle completed
            else go nextNode' newAcc  -- Continue traversing
        Nothing -> return newAcc  -- This shouldn't happen in a circular list, but we handle it
